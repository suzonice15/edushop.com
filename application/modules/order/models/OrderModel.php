...
Class OrderModel extends CI_Model
{
	...
	/*# single order result by order_id #*/
	public function orderReport($from,$to)
	{
		$this->db->select('*');
		$this->db->from('order_data');
		$this->db->where('order_status', 'completed');
		$this->db->where('modified_time >=', $from);
		$this->db->where('modified_time <=', $to);
		$result = $this->db->get();
		return $result->result();
	}
	
public function  sellProductReport($product_id){
	$this->db->select('*');
	$this->db->from('order_data');
	$this->db->join('order_report', 'order_report.order_id =order_data.order_id');
	$this->db->where('order_status', 'completed');
	$this->db->where('order_report.product_id', $product_id);
	//$this->db->where('modified_time <=', $to);
	$result = $this->db->get();
	return $result->result();
}
	public function  sellProductReportByDate($product_id,$from,$to){
		$this->db->select('*');
		$this->db->from('order_data');
		$this->db->join('order_report', 'order_report.order_id =order_data.order_id');
		$this->db->where('order_status', 'completed');
		$this->db->where('order_report.product_id', $product_id);
		$this->db->where('order_report.modified_time <=', $to);
		$this->db->where('order_report.modified_time >=', $from);
		$result = $this->db->get();
		return $result->result();
	}
