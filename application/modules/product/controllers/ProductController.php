...
$data['category_lists']= $this->MainModel->getAllData('', 'category', 'category_id,category_title', 'category_id ASC');
		...
		if($this->input->post()){
			...
			$data['category_id']=$this->input->post('category_id');
			 $query="SELECT DISTINCT(product_name),product_price,created_time,purchase_price,status,product.product_id,purchase_price,discount_price,sku,product_stock,product_title  FROM `product` join term_relation on term_relation.product_id=product.product_id join category on category.category_id=term_relation.term_id WHERE term_relation.term_id =$category_id ";
...
//		}
	...
	public function store()
	{
		...
		$row_data['product_stock'] = $this->input->post('product_stock');
			...
			else
			{
				if((($_FILES["featured_image"]["type"]=="image/jpg") || ($_FILES["featured_image"]["type"]=="image/jpeg") || ($_FILES["featured_image"]["type"]=="image/png") || ($_FILES["featured_image"]["type"]=="image/gif")))
				{
					...
					else
					{
						...
						$media_data 					= array();
						$media_data['media_title'] 		= $row_data['product_title'].$row_data['sku'];
						$media_data['media_path'] 		= $uploaded_file_path;
						$media_data['created_time']		= date("Y-m-d H:i:s");
						$media_data['modified_time']	= date("Y-m-d H:i:s");
